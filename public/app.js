...
async function openSupportOnce(existing){
  ...
  evidence(state.case.checkpoint);const oldImage=$('incident-image');oldImage?.remove();if(state.case.browserEvidence){const img=document.createElement('img');img.id='incident-image';img.alt='本次反馈保存的浏览器现场截图';img.src=`/api/cases/${state.case.id}/image`;$('evidence-preview').append(img);}await refreshTimeline();$('description').focus();
}
...
async function refreshTimeline(){if(!state.case)return;const result=await api(`/api/cases/${state.case.id}/events`);$('timeline').replaceChildren(...result.events.map(e=>{const li=text('li',messages[e.action]||'调查状态已更新');li.append(text('time',new Date(e.occurred_at).toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})));return li;}));if(result.customerMessage){let answer=$('diagnosis-answer');if(!answer){answer=text('div','','receipt');answer.id='diagnosis-answer';$('timeline').after(answer);}answer.textContent=result.customerMessage;}if(result.agentError)$('support-error').textContent=result.agentError;if(['completed','needs_review'].includes(result.status))clearInterval(state.poll);}
