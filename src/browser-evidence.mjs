...
export function sanitizeBrowserEvidence(value){
  if(value?.source!=='chrome-debugger-extension'||value.mimeType!=='image/jpeg'||typeof value.image!=='string'||value.image.length>350000||!/^\/9j\/[A-Za-z0-9+/]*={0,2}$/.test(value.image))return null;
  ...
  return {image:value.image,metadata:{source:'chrome-debugger-extension',trust:'client-supplied; not independently attested',mimeType:'image/jpeg',imageSize,capturedAt:new Date(stamp).toISOString(),viewport,network}};
}
